...
const PaginationButtons = ({ paginationData }) => {
  ...
  const prevHref =
    paginationData.prevPage === 1
      ? `/authors/${query.username[0]}`
      : `/authors/${query.username[0]}/pages/${paginationData.prevPage}`;
  const nextHref = `/authors/${query.username[0]}/pages/${paginationData.nextPage}`;

  return (
    <>
      <Head>
        {paginationData.prevPage && <link rel="prev" href={prevHref} />}
        {paginationData.nextPage && <link rel="next" href={nextHref} />}
      </Head>
